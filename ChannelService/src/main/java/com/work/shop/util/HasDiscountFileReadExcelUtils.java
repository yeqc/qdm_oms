...
public class HasDiscountFileReadExcelUtils {

	public final static String REG_DIGIT = "\\d{1,10}\\.*\\d{0,2}"; // 判断字符串是否全为数字，否继续下次操作
	private static final Logger logger = Logger.getLogger(HasDiscountFileReadExcelUtils.class);
		...
		try {
			...
			while ((nextLine = reader.readNext()) != null) {
				if (j != 0) {
					...
					String gs = "";
					gs = new BigDecimal(goodsSn).toString();
					if (map.get(gs) != null) {
					...
					// 1、 支持会员打折
					String hasDiscount = nextLine[1];
					if (StringUtil.isEmpty(hasDiscount)) {
						continue;
					}
					ticketInfo.setHasDiscount("Y".equals(hasDiscount.trim()) ? "true"
							: "false");
	...
	public static List<TicketInfo> readXls(InputStream is, String ticketCode, String shopCode, StringBuffer sb) throws Exception {
		...
		for (int numSheet = 0; numSheet < hssfWorkbook.getNumberOfSheets(); numSheet++) {
			...
			for (int rowNum = 1; rowNum <= hssfSheet.getLastRowNum(); rowNum++) {
				...
				// 1、 支持会员打折
				HSSFCell hasDiscount = hssfRow.getCell((short) 1);
				if (hasDiscount == null) {
					continue;
				}
				ticketInfo.setHasDiscount("Y".equals(getValue(hasDiscount)) ? "true" : "false");
