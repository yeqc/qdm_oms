...
public class FreightPayerFileReadExcelUtils {

	public final static String REG_DIGIT = "\\d{1,10}\\.*\\d{0,2}"; // 判断字符串是否全为数字，否继续下次操作
	private static final Logger logger = Logger.getLogger(FreightPayerFileReadExcelUtils.class);
	 ...
	 * 输入/输出(i/o)异常
	 */
	public static List<TicketInfo> readCsv(InputStream is, String ticketCode, String shopCode, StringBuffer sb) throws Exception {
		...
		try {
			...
			while ((nextLine = reader.readNext()) != null) {
				if (j != 0) {
					...
					// 1、 运费承担方式
					String freightPayer  = nextLine[1];
					if (StringUtil.isEmpty(freightPayer)) {
						continue;
					}
					ticketInfo.setFreightPayer(freightPayer.trim());
	...
	public static List<TicketInfo> readXls(InputStream is, String ticketCode, String shopCode, StringBuffer sb) throws Exception {
		...
		for (int numSheet = 0; numSheet < hssfWorkbook.getNumberOfSheets(); numSheet++) {
			...
			for (int rowNum = 1; rowNum <= hssfSheet.getLastRowNum(); rowNum++) {
				...
				// 1、 运费承担方式
				HSSFCell freightPayer = hssfRow.getCell((short) 1);
				if (freightPayer == null) {
					continue;
				}
				ticketInfo.setFreightPayer(getValue(freightPayer));
