...
@Service
public class TicketInfoPriceReadExcelUtils {
	...
	public List<TicketInfo> readCsv(File file, String ticketCode,
			String shopCode, StringBuffer sb1, StringBuffer sb2, String type)
			throws IOException {
		...
		try {
			...
			while ((nextLine = reader.readNext()) != null) {
				if (j != 0) {
					...
					String gs = goodsSn;
					if (map.get(gs) != null) {
						sb1.append("第" + (i) + "行[" + map.get(gs) + "]已存在,");
						break;
					}
					map.put(gs, gs);
					...
					String newPrice = nextLine[1];// 商品新价格
					// 变更基础数据表
					...
					if (total == 0) {
						sb1.append("第" + (i) + "行[" + gs + "]商品没有添加到该店铺,");
						continue;
					}
					/*BGproductGoods pg = productGoodsService.selectByGoodsSn(goodsSn);
					if (null == pg) {
						sb1.append("第" + (i) + "行[" + map.get(gs) + "]商品编码不存在,");
						continue;
					}
					if (!type.equals("2") && null != pg && pg.getProtectPrice().compareTo(
									new BigDecimal(newPrice)) > 0) {
						sb2.append("第" + (i) + "行[" + map.get(gs) + "]新价低于保护价,");
						continue;
					}
					*/
					if (newPrice == null || !isDigit(newPrice)) {
						sb1.append("第" + (i) + "行[" + map.get(gs) + "]新价异常,");
						continue;
					}
					ticketInfo.setNewPrice(BigDecimal.valueOf(Double
							.parseDouble(newPrice)));// 商品新价格
...
//               
//               
               HSSFCell newPrice = hssfRow.getCell((short) 1);//商品新价格
               if (newPrice == null) {
            	   sb.append("第"+(rowNum+1)+"行["+map.get(gs)+"]新价异常,");
                   continue;
               }
               if(!isDigit(getValue(newPrice))){
            	   sb.append("第"+(rowNum+1)+"行["+map.get(gs)+"]新价异常,");
            	   continue;
               }
               ticketInfo.setNewPrice(BigDecimal.valueOf(Double.parseDouble(getValue(newPrice))));//商品新价格  
