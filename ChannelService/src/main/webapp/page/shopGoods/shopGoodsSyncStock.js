	...
	//  var shopNameStr = GetCookie(name);
				...
				return {
					init : function() {
					 ...
					 var typeOptionCombo = new Ext.form.ComboBox( {
							id : 'channelGoodsTicketVo2.isTiming',
							...
							forceSelection : true,
							emptyText : '请选择执行类型',
							name : 'channelGoodsTicketVo2.isTiming',
							editable : false,
							hiddenName : 'channelGoodsTicketVo2.isTiming',
							triggerAction : 'all', 
							fieldLabel : '执行类型',
							width : 150
						});
					 ...
					 //经营店铺选择框
					  shopSelectOption = new Ext.form.ComboBox( {
							id : 'channelGoodsTicketVo2.shopCode',
					 ...
					 var payStatusOption = new Ext.form.ComboBox( {
							id : 'channelGoodsTicketVo2.ticketStatus',
							store : new Ext.data.SimpleStore( {
							...
							forceSelection : true,
							emptyText : '请选择单据状态',
							name : 'channelGoodsTicketVo2.ticketStatus',
							editable : false,
							hiddenName : 'channelGoodsTicketVo2.ticketStatus',
							triggerAction : 'all', 
							fieldLabel : '单据状态',
							width : 150
						});
									...
									items : [ {
										...
										items : [{
													...
													items : [shopSelectOption,{
														id : 'channelGoodsTicketVo2.ticketCode',
														xtype : 'textfield',
														labelAlign : 'left',
														fieldLabel : '调整单编号',
														name : 'channelGoodsTicketVo2.ticketCode', 
														blankText : '调整单编号不能为空',
														width : '142'
													}]
												},{
				        ...
				        channelGoodsTicketStore.on('beforeload', function(){
				            Ext.apply(this.baseParams, {ticketType: 9, shopCode:shopCodeStr});   
				        });
						...
						tbar = [{
							...
							iconCls : 'add',
							handler : function() {
							FormEditWin.showAddDirWin("popWins",basePath+"/custom/shopGoods/addShopGoodsSyncStock.spmvc","pop_message_winID","生成调整单",860,560);
							}
						    },{ 
							...
							listeners : {
					            'rowdblclick': function(thisgrid ,rowIndex ,e){
									...
									var id = record.data['id']; 
									 FormEditWin.showAddDirWin("popWins",basePath+"/custom/ticketInfo/updateOrSearchSyncStockTicketInfo.spmvc?id="+id,"pop_message_winID","查看/更新调整单",860,560);
					            }
					        }
						});
				   ...
				   deleteObject : function() { //删除整个panel里面被选择的行数据
					...
					search : function() {
					  var outStock = {};					      
					      outStock["isTiming"] = Ext.getCmp("channelGoodsTicketVo2.isTiming").getValue();
					      outStock["shopCode"] = Ext.getCmp("channelGoodsTicketVo2.shopCode").getValue();
						  outStock["ticketStatus"] = Ext.getCmp("channelGoodsTicketVo2.ticketStatus").getValue();
					      outStock["ticketCode"] = Ext.getCmp("channelGoodsTicketVo2.ticketCode").getValue(); //单据编号
						  channelGoodsTicketGrid.store.baseParams = outStock;
					//	  Ext.Msg.alert(Ext.getCmp("channelGoodsTicketVo2.ticketCode").getValue());//1上下架单据类型
						  channelGoodsTicketGrid.store.load({params : {start : 0, limit : pageSize,ticketType: 9 }});
					} 
			...
			function clickResetButton() {
				...
				shopSelectOption.store.on('load', function(store, record, opts) {
 					Ext.getCmp('channelGoodsTicketVo2.shopCode').setValue(shopCodeStr); //获取登录人的店铺code
 				});
