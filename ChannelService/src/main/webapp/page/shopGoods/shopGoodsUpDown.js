	...
	//  var shopNameStr = GetCookie(name);
				...
				return {
					init : function() {
					 ...
					 var typeOptionCombo = new Ext.form.ComboBox( {
							id : 'channelGoodsTicketVo.isTiming',
							...
							forceSelection : true,
							emptyText : '请选择执行类型',
							name : 'channelGoodsTicketVo.isTiming',
							editable : false,
							hiddenName : 'channelGoodsTicketVo.isTiming',
							triggerAction : 'all', 
							fieldLabel : '执行类型',
							width : 150
						});
					 ...
					 //经营店铺选择框
					  shopSelectOption = new Ext.form.ComboBox( {
							id : 'channelGoodsTicketVo.shopCode',
					 ...
					 var payStatusOption = new Ext.form.ComboBox( {
							id : 'channelGoodsTicketVo.ticketStatus',
							store : new Ext.data.SimpleStore( {
							...
							forceSelection : true,
							emptyText : '请选择单据状态',
							name : 'channelGoodsTicketVo.ticketStatus',
							editable : false,
							hiddenName : 'channelGoodsTicketVo.ticketStatus',
							triggerAction : 'all', 
							fieldLabel : '单据状态',
							width : 150
						});
									...
									items : [ {
										...
										items : [{
													...
													items : [shopSelectOption,{
														id : 'channelGoodsTicketVo.ticketCode',
														xtype : 'textfield',
														labelAlign : 'left',
														fieldLabel : '调整单编号',
														name : 'channelGoodsTicketVo.ticketCode', 
														blankText : '调整单编号不能为空',
														width : '142'
													}]
												},{
				        ...
				        channelGoodsTicketStore.on('beforeload', function(){
				            Ext.apply(this.baseParams, {ticketType: 1, shopCode:shopCodeStr});   
				        });
						...
						tbar = [{
							...
							iconCls : 'add',
							handler : function() {
							FormEditWin.showAddDirWin("popWins",basePath+"/custom/shopGoods/addShopGoodsUpDownPage.spmvc","pop_message_winID","生成调整单",860,560);
							}
						    },{ 
				   ...
				   deleteObject : function() { //删除整个panel里面被选择的行数据
					...
					search : function() {
					  var outStock = {};					      
					      outStock["isTiming"] = Ext.getCmp("channelGoodsTicketVo.isTiming").getValue();
					      outStock["shopCode"] = Ext.getCmp("channelGoodsTicketVo.shopCode").getValue();
						  outStock["ticketStatus"] = Ext.getCmp("channelGoodsTicketVo.ticketStatus").getValue();
					      outStock["ticketCode"] = Ext.getCmp("channelGoodsTicketVo.ticketCode").getValue(); //单据编号
					...
					//	  Ext.Msg.alert(Ext.getCmp("channelGoodsTicketVo.ticketCode").getValue());//1上下架单据类型
						  channelGoodsTicketGrid.store.load({params : {start : 0, limit : pageSize,ticketType: 1 }});
					} 
			...
			function clickResetButton() {
				...
				shopSelectOption.store.on('load', function(store, record, opts) {
 					Ext.getCmp('channelGoodsTicketVo.shopCode').setValue(shopCodeStr); //获取登录人的店铺code
 				});
