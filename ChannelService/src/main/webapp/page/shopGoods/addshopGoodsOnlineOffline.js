...
Ext.onReady(function() {
	...
	TicketInfo= function() {
					...
					init : function() {
						...
						//渠道信息选择框
						 channelInfoCombo = new Ext.form.ComboBox({
							id : 'addshopGoodsOnlineOffline.channelCode',
					 ...
					 var typeOptionCombo = new Ext.form.ComboBox( {
							id : 'addShopGoodsOnlineOffline.ticketType',
							...
							blankText : '请选择调整类型',
							emptyText : '请选择调整类型',
							name : 'addShopGoodsOnlineOffline.ticketType',
							editable : false,
							hiddenName : 'addShopGoodsOnlineOffline.ticketType',
							triggerAction : 'all', 
							fieldLabel : '调整类型',
							width : 150
						});
					 ...
					 //经营店铺选择框
					  shopSelectOption = new Ext.form.ComboBox( {
							id : 'addShopGoodsOnlineOffline.shopcode',
							store : new Ext.data.Store({
								proxy : new Ext.data.HttpProxy({
					 ...
					 var isTimingSelectOption = new Ext.form.ComboBox( {
							id : 'addShopGoodsOnlineOffline.isTiming',
							store : new Ext.data.SimpleStore( {
								data : [ [ '0', '否' ], [ '1', '是' ]],  //, [ '1', '是' ]
								fields : [ 'text', 'filed' ]
							}),
							...
							listeners:{
					                'select': function(){
					                        if(Ext.getCmp("addShopGoodsOnlineOffline.isTiming").getValue() == '0')
					                        {    
						...
						ticketInfoForm = new Ext.FormPanel(
								{
									...
									items : [ {
										...
										items : [{
													...
													columnWidth:0.3,
													items : [{
														id : 'addShopGoodsOnlineOffline.ticketCode',
														xtype : 'textfield',
														fieldLabel : '调整单编号',
														name : 'addShopGoodsOnlineOffline.ticketCode', 
														allowBlank:false,
														blankText : '调整单编号不能为空',
														readOnly:true,
						...
						ticketInfoStore.on('beforeload', function(){
				            var keyWord = Ext.getCmp("addShopGoodsOnlineOffline.ticketCode").getValue();
						...
						tbar = [{
						    ...
						    ,{ 
							    ...
							    },'<a style="color:red" href = "'+ basePath + '/custom/fileUpload/fileDownloadCsv.spmvc?filePath=onlineOffline.csv' + '">模板下载</a>'
						    ];
			...
			function upload (){ //导入excel以后就与调整单绑定传入数据库保存
				 var ticketCode  = Ext.getCmp("addShopGoodsOnlineOffline.ticketCode").getValue(); //单据编号
				 var time = new Date().format('Y-m-d H:i:s');
				 var excetTime = $("#channelGoodsTicket_excuteTime").val() ;//执行时间
				 var isTiming =   Ext.getCmp("addShopGoodsOnlineOffline.isTiming").getValue();//1为定时执行
				 var channelCode = Ext.getCmp("addshopGoodsOnlineOffline.channelCode").getValue(); //渠道code
				 var shopCode = Ext.getCmp("addShopGoodsOnlineOffline.shopcode").getValue(); //店铺名称code
				...
				FormEditWin.showAddDirWin(null,basePath+"/fileUpload.html?uploadUrl="+basePath+"custom/ticketInfo/inportOnlineOfflineChannelGoods.spmvc"+params,"picADWinID","上传",480,300);
			}
			...
			function addChannelGoodsTicket() { 
				      var ticketCode  = Ext.getCmp("addShopGoodsOnlineOffline.ticketCode").getValue(); //单据编号
				      var shopCode = Ext.getCmp("addShopGoodsOnlineOffline.shopcode").getValue(); //店铺名称code
				      var isTimming = Ext.getCmp("addShopGoodsOnlineOffline.isTiming").getValue(); //是否定时执行
						 ...
						 Ext.Ajax
								.request( {
									...
									params : {
										...
										channelCode:Ext.getCmp("addshopGoodsOnlineOffline.channelCode").getValue(),
										shopCode:shopCode,
										ticketType:8, //上下线类型
	  ...
	  function runChannelGoodsTicket() {//点击查看/更新页面展示		
		    // alert("shopCodeStr="+shopCodeStr);
					Ext.getCmp('addShopGoodsOnlineOffline.ticketCode').setValue($("#addNewTicketCode").val()); 
					
					shopSelectOption.store.on('load', function(store, record, opts) {
	 					Ext.getCmp('addShopGoodsOnlineOffline.shopcode').setValue(shopCodeStr); 
	 				});
					shopSelectOption.store.load();
					
					channelInfoCombo.store.on('load', function(store, record, opts) {
	 					Ext.getCmp('addshopGoodsOnlineOffline.channelCode').setValue(chanelCodeStr); 
	 				});
					channelInfoCombo.store.load();
	 			//	Ext.getCmp('addShopGoodsOnlineOffline.shopcode').setValue($("#addNewShopCode").val());
					$("#channelGoodsTicket_excuteTime").val($("#addNewExcetTime").val());
					//是否定时执行($("#addNewIsTiming").val())
					Ext.getCmp('addShopGoodsOnlineOffline.isTiming').setValue($("#addNewIsTiming").val());
					
					$("#channelGoodsTicket_excuteTime").val($("#addNewExcetTime").val()) ;//执行时间
					Ext.getCmp('addShopGoodsOnlineOffline.isTiming').setValue("0");
					var isTiming = $("#addNewIsTiming").val();
					if (null == isTiming || isTiming == '') {
						isTiming = '0';
					}
					Ext.getCmp('addShopGoodsOnlineOffline.isTiming').setValue(isTiming);
					if(isTiming == '0'){
						 Ext.getCmp("excuteTimeNo").disable(); 
						 $("#channelGoodsTicket_excuteTime").val("") ;//执行时间
					}
