...
Ext.onReady(function() {
	...
	TicketInfo= function() {
				...
				return {
					init : function() {
						...
						//渠道信息选择框
						 channelInfoCombo = new Ext.form.ComboBox({
							id : 'OutChannelShop2.channelCode',
					 ...
					 var typeOptionCombo = new Ext.form.ComboBox( {
							id : 'channelGoodsTicket2.ticketType',
							...
							blankText : '请选择调整类型',
							emptyText : '请选择调整类型',
							name : 'channelGoodsTicket2.ticketType',
							editable : false,
							hiddenName : 'channelGoodsTicket2.ticketType',
							triggerAction : 'all', 
							fieldLabel : '调整类型',
							width : 150
						});
					 ...
					 //经营店铺选择框
					  shopSelectOption = new Ext.form.ComboBox( {
							id : 'channelGoodsTicket2.shopCode',
							store : new Ext.data.Store({
								proxy : new Ext.data.HttpProxy({
					 ...
					 var isTimingSelectOption = new Ext.form.ComboBox( {
							id : 'channelGoodsTicket2.isTiming',
							store : new Ext.data.SimpleStore( {
								data : [ [ '0', '否' ], [ '1', '是' ]],  //, [ '1', '是' ]
								fields : [ 'text', 'filed' ]
							}),
							...
							listeners:{
					                'select': function(){
					                        if(Ext.getCmp("channelGoodsTicket2.isTiming").getValue() == '0')
					                        {    
						...
						ticketInfoForm = new Ext.FormPanel(
								{
									...
									items : [ {
										...
										items : [{
													...
													columnWidth:0.3,
													items : [{
														id : 'channelGoodsTicket2.ticketCode',
														xtype : 'textfield',
														fieldLabel : '调整单编号',
														name : 'channelGoodsTicket2.ticketCode', 
														allowBlank:false,
														blankText : '调整单编号不能为空',
														readOnly:true,
						...
						ticketInfoStore.on('beforeload', function(){
				            var keyWord = Ext.getCmp("channelGoodsTicket2.ticketCode").getValue();
					...
					show : function() {
						...
						couponPanel = new Ext.Panel( {
							renderTo : 'shopGoodsSyncStock-grid',
			...
			function upload (){ //导入excel以后就与调整单绑定传入数据库保存
				 var ticketCode  = Ext.getCmp("channelGoodsTicket2.ticketCode").getValue(); //单据编号
				 var time = new Date().format('Y-m-d H:i:s');
				 var excetTime = $("#channelGoodsTicket_excuteTime").val() ;//执行时间
				 var isTiming =   Ext.getCmp("channelGoodsTicket2.isTiming").getValue();//1为定时执行
				 var channelCode = Ext.getCmp("OutChannelShop2.channelCode").getValue(); //渠道code
				 var shopCode = Ext.getCmp("channelGoodsTicket2.shopCode").getValue(); //店铺名称code
				...
				FormEditWin.showAddDirWin(null,basePath+"/fileUpload.html?uploadUrl="+basePath+"custom/ticketInfo/inportChannelGoodsSyncStock.spmvc"+params,"picADWinID","上传",480,300);
			}
			...
			function addChannelGoodsTicket() { 
				      var ticketCode  = Ext.getCmp("channelGoodsTicket2.ticketCode").getValue(); //单据编号
				      var shopCode = Ext.getCmp("channelGoodsTicket2.shopCode").getValue(); //店铺名称code
				      var isTimming = Ext.getCmp("channelGoodsTicket2.isTiming").getValue(); //是否定时执行
						 ...
						 Ext.Ajax
								.request( {
									...
									params : {
										...
										channelCode:Ext.getCmp("OutChannelShop2.channelCode").getValue(),
										shopCode:shopCode,
										ticketType:1, //上下架类型
	  ...
	  function runChannelGoodsTicket() {//点击查看/更新页面展示		
		    // alert("shopCodeStr="+shopCodeStr);
					Ext.getCmp('channelGoodsTicket2.ticketCode').setValue($("#addNewTicketCode").val()); 
					
					shopSelectOption.store.on('load', function(store, record, opts) {
	 					Ext.getCmp('channelGoodsTicket2.shopCode').setValue(shopCodeStr); 
	 				});
					shopSelectOption.store.load();
					
					channelInfoCombo.store.on('load', function(store, record, opts) {
	 					Ext.getCmp('OutChannelShop2.channelCode').setValue(chanelCodeStr); 
	 				});
					channelInfoCombo.store.load();
	 			//	Ext.getCmp('channelGoodsTicket2.shopCode').setValue($("#addNewShopCode").val());
					$("#channelGoodsTicket_excuteTime").val($("#addNewExcetTime").val());
					//是否定时执行($("#addNewIsTiming").val())
					Ext.getCmp('channelGoodsTicket2.isTiming').setValue($("#addNewIsTiming").val());
					
					$("#channelGoodsTicket_excuteTime").val($("#addNewExcetTime").val()) ;//执行时间
					Ext.getCmp('channelGoodsTicket2.isTiming').setValue("0");
					var isTiming = $("#addNewIsTiming").val();
					if (null == isTiming || isTiming == '') {
						isTiming = '0';
					}
					Ext.getCmp('channelGoodsTicket2.isTiming').setValue(isTiming);
					if(isTiming == '0'){
						 Ext.getCmp("excuteTimeNo").disable(); 
						 $("#channelGoodsTicket_excuteTime").val("") ;//执行时间
					}
