Ext.onReady(function() {
	...
	ChannelShopGoodsPriceVo= function() {
				...
				return {
					...
					var typeOptionCombo = new Ext.form.ComboBox( {
							id : 'ChannelShopGoodsPriceVo.isTiming',
							...
							forceSelection : true,
							emptyText : '请选择执行类型',
							name : 'ChannelShopGoodsPriceVo.isTiming',
							editable : false,
							hiddenName : 'ChannelShopGoodsPriceVo.isTiming',
							triggerAction : 'all', 
							fieldLabel : '执行类型',
							width : 150
						});
					 ...
					 shopSelectOption = new Ext.form.ComboBox( {
							id : 'ChannelShopGoodsPriceVo.shopCode',
					 ...
					 var payStatusOption = new Ext.form.ComboBox( {
							id : 'ChannelShopGoodsPriceVo.ticketStatus',
							store : new Ext.data.SimpleStore( {
							...
							forceSelection : true,
							emptyText : '请选择单据状态',
							name : 'ChannelShopGoodsPriceVo.ticketStatus',
							editable : false,
							hiddenName : 'ChannelShopGoodsPriceVo.ticketStatus',
							triggerAction : 'all', 
							fieldLabel : '单据状态',
							width : 150
						});
									...
									items : [ {
										...
										items : [{
													...
													items : [shopSelectOption,{
														id : 'ChannelShopGoodsPriceVo.ticketCode',
														xtype : 'textfield',
														fieldLabel : '调整单编号',
														name : 'ChannelShopGoodsPriceVo.ticketCode', 
														blankText : '调整单编号不能为空',
														width : '142'
													} ]
												},{
						...
						channelGoodsTicketColumnGrid = new Ext.grid.ColumnModel( [
								...
								},	
								{
									...
									dataIndex : 'ticketStatus',
									renderer:function(e){
									       var msg = "<input type='button' value='导出调整单' onclick='ChannelShopGoodsPriceVo.exportRecord();' />"; 			
									        if(e=='3'){
									        	msg = "<input type='button' value='导出执行结果' onclick='ChannelShopGoodsPriceVo.exportRecord();' />";
									        }
									       return msg;
									}
				        ...
				        channelGoodsTicketStore.on('beforeload', function(){
				         //   var keyword = getKeyword();
				            Ext.apply(this.baseParams, {ticketType: 0,shopCode:shopCodeStr});   
				        });
						...
						tbar = [{
							...
							iconCls : 'add',
							handler : function() {
							FormEditWin.showAddDirWin("popWins",basePath+"/custom/shopGoodsPrice/addShopGoodsPricePage.spmvc","pops_message_winID","生成调整单",860,560);
							}
						    },{ 
							...
							iconCls : 'add',
							handler : function() { 
								ChannelShopGoodsPriceVo.reviewObject();
						  ...
						  /**  ,{
								...
								iconCls : 'delete',
								handler:function(){
									ChannelShopGoodsPriceVo.unReviewObject();
								}
						    }**/
						    ,{
							...
							iconCls : 'delete',
							handler:function(){
								ChannelShopGoodsPriceVo.deleteObject();
							}
						}];
							...
							listeners : {
					            'rowdblclick': function(thisgrid ,rowIndex ,e){
									...
									var id = record.data['id']; 
									 FormEditWin.showAddDirWin("popWins",basePath+"/custom/shopGoodsPrice/updateOrSearchTicketInfo.spmvc?id="+id,"pop_message_winID","查看/更新调整单",860,560);
					            }
					        }
						});
					},
					...
					exportRecord:function(){//导出本行的数据
						...
						if (selModel.hasSelection()) {
							...
							var id = records[0].get("id");														
							if(id != ""){//批量删除
								 var path = basePath + '/custom/shopGoods/exportChannelGoodsTickets.spmvc?id='+id+"&type=price";
				   ...
				   deleteObject : function() { //删除整个panel里面被选择的行数据
					...
					search : function() {
					  var outStock = {};					      
					      outStock["isTiming"] = Ext.getCmp("ChannelShopGoodsPriceVo.isTiming").getValue();
					      outStock["shopCode"] = Ext.getCmp("ChannelShopGoodsPriceVo.shopCode").getValue();
						  outStock["ticketStatus"] = Ext.getCmp("ChannelShopGoodsPriceVo.ticketStatus").getValue();
					      outStock["ticketCode"] = Ext.getCmp("ChannelShopGoodsPriceVo.ticketCode").getValue(); //单据编号
						  channelGoodsTicketGrid.store.baseParams = outStock;
					//	  Ext.Msg.alert(Ext.getCmp("ChannelShopGoodsPriceVo.ticketCode").getValue());
						  channelGoodsTicketGrid.store.load({params : {start : 0, limit : pageSize,ticketType: 0}});
					} 
			...
			function runChannelGoodsTicketList() {
				ChannelShopGoodsPriceVo.init(); 
				ChannelShopGoodsPriceVo.show();
				setResize();
				shopSelectOption.store.on('load', function(store, record, opts) {
 					Ext.getCmp('ChannelShopGoodsPriceVo.shopCode').setValue(shopCodeStr); //获取登录人的店铺code
 				});
