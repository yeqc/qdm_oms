...
//								}
						...
						tbar = [{
						    ...
						    ,{ 
							    ...
							    },'<a style="color:red" href = "'+ basePath + '/custom/fileUpload/fileDownloadCsv.spmvc?filePath=price.csv' + '">模板下载</a>'
						    ];
					...
					show : function() {
						...
						couponPanel = new Ext.Panel( {
							renderTo : 'addShopGoodsPrice-grid',
					...
					doAfter : function(data){//上传后的数据返回
						...
						var json = Ext.util.JSON.decode(data);
                             ...
                             parent.ChannelShopGoodsPriceVo.refresh();
			...
			function upload (){ //导入excel以后就与调整单绑定传入数据库保存
				...
				FormEditWin.showAddDirWin(null,basePath+"/fileUpload.html?uploadUrl="+basePath+"custom/shopGoodsPrice/inportChannelGoodsPrice.spmvc"+params,"picADWinID","上传",480,300);
			}
