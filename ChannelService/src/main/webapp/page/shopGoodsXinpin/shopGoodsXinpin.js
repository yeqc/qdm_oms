	...
	//  var shopNameStr = GetCookie(name);
				...
				return {
					init : function() {
					 ...
					 var typeOptionCombo = new Ext.form.ComboBox( {
							...
							forceSelection : true,
							emptyText : '请选择执行类型',
							name : 'shopGoodsXinpin.isTiming',
							editable : false,
							hiddenName : 'shopGoodsXinpin.isTiming',
							triggerAction : 'all', 
							fieldLabel : '执行类型',
							width : 150
						});
					 ...
					 var payStatusOption = new Ext.form.ComboBox( {
							id : 'shopGoodsXinpin.ticketStatus',
							store : new Ext.data.SimpleStore( {
							...
							forceSelection : true,
							emptyText : '请选择单据状态',
							name : 'shopGoodsXinpin.ticketStatus',
							editable : false,
							hiddenName : 'shopGoodsXinpin.ticketStatus',
							triggerAction : 'all', 
							fieldLabel : '单据状态',
							width : 150
						});
									...
									items : [ {
										...
										items : [{
													...
													items : [shopSelectOption,{
														id : 'shopGoodsXinpin.ticketCode',
														xtype : 'textfield',
														labelAlign : 'left',
														fieldLabel : '调整单编号',
														name : 'shopGoodsXinpin.ticketCode', 
														blankText : '调整单编号不能为空',
														width : '142'
													}]
												},{
						...
						channelGoodsTicketProxy = new Ext.data.HttpProxy( {
							url : basePath+"/custom/shopGoodsXinpin/shopGoodsXinpin.spmvc",
							method : "post"
						});
						//Reader 读json中数据
				        ...
				        channelGoodsTicketStore.on('beforeload', function(){
				            Ext.apply(this.baseParams, {ticketType: 11, shopCode:shopCodeStr});   
				        });
						...
						tbar = [{
							...
							iconCls : 'add',
							handler : function() {
							FormEditWin.showAddDirWin("popWins",basePath+"/custom/shopGoodsXinpin/addshopGoodsXinpinPage.spmvc","pop_message_winID","生成调整单",860,560);
							}
						    },{ 
							...
							listeners : {
					            'rowdblclick': function(thisgrid ,rowIndex ,e){
									...
									var id = record.data['id']; 
									 FormEditWin.showAddDirWin("popWins",basePath+"/custom/shopGoodsXinpin/updateOrSearchXinpinTicketInfo.spmvc?id="+id,"pop_message_winID","查看/更新调整单",860,560);
					            }
					        }
						});
				   ...
				   deleteObject : function() { //删除整个panel里面被选择的行数据
					...
					search : function() {
					  var outStock = {};					      
					      outStock["isTiming"] = Ext.getCmp("shopGoodsXinpin.isTiming").getValue();
					      outStock["shopCode"] = Ext.getCmp("shopGoodsXinpin.shopCode").getValue();
						  outStock["ticketStatus"] = Ext.getCmp("shopGoodsXinpin.ticketStatus").getValue();
					      outStock["ticketCode"] = Ext.getCmp("shopGoodsXinpin.ticketCode").getValue(); //单据编号
						  channelGoodsTicketGrid.store.baseParams = outStock;
						  //Ext.Msg.alert(Ext.getCmp("shopGoodsXinpin.ticketCode").getValue());//1上下架单据类型
						  channelGoodsTicketGrid.store.load({params : {start : 0, limit : pageSize,ticketType: 11 }});
					} 
			...
			function clickResetButton() {
				...
				shopSelectOption.store.on('load', function(store, record, opts) {
 					Ext.getCmp('shopGoodsXinpin.shopCode').setValue(shopCodeStr); //获取登录人的店铺code
 				});
