	...
	TicketInfo= function() {
				...
				return {
					init : function() {
						...
						//渠道信息选择框
						 channelInfoCombo = new Ext.form.ComboBox({
							id : 'addshopGoodsXinpin.channelCode',
					...
					/* var typeOptionCombo = new Ext.form.ComboBox( {
							id : 'addshopGoodsXinpin.ticketType',
							...
							blankText : '请选择调整类型',
							emptyText : '请选择调整类型',
							name : 'addshopGoodsXinpin.ticketType',
							editable : false,
							hiddenName : 'addshopGoodsXinpin.ticketType',
							triggerAction : 'all', 
							fieldLabel : '调整类型',
							width : 150
						});*/
					 ...
					 //经营店铺选择框
					  shopSelectOption = new Ext.form.ComboBox( {
							id : 'addshopGoodsXinpin.shopcode',
							store : new Ext.data.Store({
								proxy : new Ext.data.HttpProxy({
					 ...
					 var isTimingSelectOption = new Ext.form.ComboBox( {
							id : 'addshopGoodsXinpin.isTiming',
							store : new Ext.data.SimpleStore( {
								data : [ [ '0', '否' ], [ '1', '是' ]],  //, [ '1', '是' ]
								fields : [ 'text', 'filed' ]
							}),
							...
							listeners:{
					                'select': function(){
					                        if(Ext.getCmp("addshopGoodsXinpin.isTiming").getValue() == '0')
					                        {    
						...
						ticketInfoForm = new Ext.FormPanel(
								{
									...
									items : [ {
										...
										items : [{
													...
													columnWidth:0.3,
													items : [{
														id : 'addshopGoodsXinpin.ticketCode',
														xtype : 'textfield',
														fieldLabel : '调整单编号',
														name : 'addshopGoodsXinpin.ticketCode', 
														allowBlank:false,
														blankText : '调整单编号不能为空',
														readOnly:true,
						...
						ticketInfoStore.on('beforeload', function(){
				            var keyWord = Ext.getCmp("addshopGoodsXinpin.ticketCode").getValue();
						...
						tbar = [{
						    ...
						    ,{ 
							    ...
							    },'<a style="color:red" href = "'+ basePath + '/custom/fileUpload/fileDownloadCsv.spmvc?filePath=xinpin.csv' + '">模板下载</a>'
						    ];
			...
			function upload (){ //导入excel以后就与调整单绑定传入数据库保存
				 var ticketCode  = Ext.getCmp("addshopGoodsXinpin.ticketCode").getValue(); //单据编号
				 var time = new Date().format('Y-m-d H:i:s');
				 var excetTime = $("#channelGoodsTicket_excuteTime").val() ;//执行时间
				 var isTiming =   Ext.getCmp("addshopGoodsXinpin.isTiming").getValue();//1为定时执行
				 var channelCode = Ext.getCmp("addshopGoodsXinpin.channelCode").getValue(); //渠道code
				 var shopCode = Ext.getCmp("addshopGoodsXinpin.shopcode").getValue(); //店铺名称code
				...
				FormEditWin.showAddDirWin(null,basePath+"/fileUpload.html?uploadUrl="+basePath+"custom/shopGoodsXinpin/inportXinpinChannelGoods.spmvc"+params,"picADWinID","上传",480,300);
			}
			...
			function addChannelGoodsTicket() { 
				      var ticketCode  = Ext.getCmp("addshopGoodsXinpin.ticketCode").getValue(); //单据编号
				      var shopCode = Ext.getCmp("addshopGoodsXinpin.shopcode").getValue(); //店铺名称code
				      var isTimming = Ext.getCmp("addshopGoodsXinpin.isTiming").getValue(); //是否定时执行
						 ...
						 Ext.Ajax
								.request( {
									...
									params : {
										...
										channelCode:Ext.getCmp("addshopGoodsXinpin.channelCode").getValue(),
										shopCode:shopCode,
										ticketType:11, //上下线类型
	  ...
	  function runChannelGoodsTicket() {//点击查看/更新页面展示		
					Ext.getCmp('addshopGoodsXinpin.ticketCode').setValue($("#addNewTicketCode").val()); 
					
					shopSelectOption.store.on('load', function(store, record, opts) {
	 					Ext.getCmp('addshopGoodsXinpin.shopcode').setValue(shopCodeStr); 
	 				});
					shopSelectOption.store.load();
					
					channelInfoCombo.store.on('load', function(store, record, opts) {
	 					Ext.getCmp('addshopGoodsXinpin.channelCode').setValue(chanelCodeStr); 
	 				});
	 			...
	 			//	Ext.getCmp('addShopGoodsOnlineOffline.shopcode').setValue($("#addNewShopCode").val());
					...
					Ext.getCmp('addshopGoodsXinpin.isTiming').setValue($("#addNewIsTiming").val());
					
					$("#channelGoodsTicket_excuteTime").val($("#addNewExcetTime").val()) ;//执行时间
					Ext.getCmp('addshopGoodsXinpin.isTiming').setValue("0");
					var isTiming = $("#addNewIsTiming").val();
					if (null == isTiming || isTiming == '') {
						isTiming = '0';
					}
					Ext.getCmp('addshopGoodsXinpin.isTiming').setValue(isTiming);
					if(isTiming == '0'){
						 Ext.getCmp("excuteTimeNo").disable(); 
						 $("#channelGoodsTicket_excuteTime").val("") ;//执行时间
					}
