Ext.onReady(function() {
	...
	ChannelShopGoodsSellingInfoVo= function() {
				...
				return {
					init : function() {
					 ...
					 var typeOptionCombo = new Ext.form.ComboBox( {
							id : 'ChannelShopGoodsSellingInfoVo.isTiming',
							...
							forceSelection : true,
							emptyText : '请选择执行类型',
							name : 'ChannelShopGoodsSellingInfoVo.isTiming',
							editable : false,
							hiddenName : 'ChannelShopGoodsSellingInfoVo.isTiming',
							triggerAction : 'all', 
							fieldLabel : '执行类型',
							width : 150
						});
					 ...
					 //经营店铺选择框
					  shopSelectOption = new Ext.form.ComboBox( {
							id : 'ChannelShopGoodsSellingInfoVo.shopCode',
					 ...
					 var payStatusOption = new Ext.form.ComboBox( {
							id : 'ChannelShopGoodsSellingInfoVo.ticketStatus',
							store : new Ext.data.SimpleStore( {
							...
							forceSelection : true,
							emptyText : '请选择单据状态',
							name : 'ChannelShopGoodsSellingInfoVo.ticketStatus',
							editable : false,
							hiddenName : 'ChannelShopGoodsSellingInfoVo.ticketStatus',
							triggerAction : 'all', 
							fieldLabel : '单据状态',
							width : 150
						});
									...
									items : [ {
										...
										items : [{
													...
													items : [shopSelectOption,{
														id : 'ChannelShopGoodsSellingInfoVo.ticketCode',
														xtype : 'textfield',
														fieldLabel : '调整单编号',
														name : 'ChannelShopGoodsSellingInfoVo.ticketCode', 
														blankText : '调整单编号不能为空',
														width : '142'
													} ]
												},{
						...
						channelGoodsTicketColumnGrid = new Ext.grid.ColumnModel( [
								...
								},	
								{
									...
									dataIndex : 'ticketStatus',
									renderer:function(e){
									       var msg = "<input type='button' value='导出调整单' onclick='ChannelShopGoodsSellingInfoVo.exportRecord();' />"; 			
									        if(e=='3'){
									        	msg = "<input type='button' value='导出执行结果' onclick='ChannelShopGoodsSellingInfoVo.exportRecord();' />";
									        }
									       return msg;
									}
				        ...
				        channelGoodsTicketStore.on('beforeload', function(){
				            Ext.apply(this.baseParams, {ticketType: 3, shopCode:shopCodeStr});   
				        });
						...
						tbar = [{
							...
							iconCls : 'add',
							handler : function() {
							FormEditWin.showAddDirWin("popWins",basePath+"/custom/shopGoodsSellingInfo/addShopGoodsPricePage.spmvc","pops_message_winID","生成调整单",860,560);
							}
						    },{ 
							...
							iconCls : 'add',
							handler : function() { 
								ChannelShopGoodsSellingInfoVo.reviewObject();
						   ...
						   /** ,{
								...
								iconCls : 'delete',
								handler:function(){
									ChannelShopGoodsSellingInfoVo.unReviewObject();
								}
						    }**/
						    ,{
							...
							iconCls : 'delete',
							handler:function(){
								ChannelShopGoodsSellingInfoVo.deleteObject();
							}
						}];
							...
							listeners : {
					            'rowdblclick': function(thisgrid ,rowIndex ,e){
									...
									var id = record.data['id']; 
									 FormEditWin.showAddDirWin("popWins",basePath+"/custom/shopGoodsSellingInfo/updateOrSearchTicketInfo.spmvc?id="+id,"pop_message_winID","查看/更新调整单",860,560);
					            }
					        }
						});
					...
					exportRecord:function(){//导出本行的数据
						...
						if (selModel.hasSelection()) {
							...
							var id = records[0].get("id");														
							if(id != ""){//批量删除
								 var path = basePath + '/custom/shopGoods/exportChannelGoodsTickets.spmvc?id='+id+"&type=sellingInfo";
				   ...
				   deleteObject : function() { //删除整个panel里面被选择的行数据
					...
					show : function() {
						...
						couponPanel = new Ext.Panel( {
							renderTo : 'shopGoodsSellingInfo-grid',
					...
					search : function() {
					  var outStock = {};					      
					      outStock["isTiming"] = Ext.getCmp("ChannelShopGoodsSellingInfoVo.isTiming").getValue();
					      outStock["shopCode"] = Ext.getCmp("ChannelShopGoodsSellingInfoVo.shopCode").getValue();
						  outStock["ticketStatus"] = Ext.getCmp("ChannelShopGoodsSellingInfoVo.ticketStatus").getValue();
					      outStock["ticketCode"] = Ext.getCmp("ChannelShopGoodsSellingInfoVo.ticketCode").getValue(); //单据编号
						  channelGoodsTicketGrid.store.baseParams = outStock;
					//	  Ext.Msg.alert(Ext.getCmp("ChannelShopGoodsSellingInfoVo.ticketCode").getValue());
						  channelGoodsTicketGrid.store.load({params : {start : 0, limit : pageSize,ticketType: 3}});
					} 
			...
			function runChannelGoodsTicketList() {
				ChannelShopGoodsSellingInfoVo.init(); 
				ChannelShopGoodsSellingInfoVo.show();
				setResize();
				shopSelectOption.store.on('load', function(store, record, opts) {
 					Ext.getCmp('ChannelShopGoodsSellingInfoVo.shopCode').setValue(shopCodeStr); //获取登录人的店铺code
 				});
