...
Ext.onReady(function() {
	...
	TicketInfo= function() {
				...
				return {
					...
					//经营店铺选择框
					 ...
					 var isTimingSelectOption = new Ext.form.ComboBox( {
							...
							allowBlank:false,
							hiddenName : 'channelGoodsTicket.isTiming',
							triggerAction : 'all', 
							fieldLabel : '是否定时执行',	
							width : 150,
							listeners:{
						...
						isTimingSelectOption.setValue('0');
						...
						tbar = [{
						    ...
						    ,{ 
							    ...
							    },'<a style="color:red" href = "'+ basePath + '/custom/fileUpload/fileDownloadCsv.spmvc?filePath=sellingInfo.csv' + '">模板下载</a>'
						    ];
					...
					show : function() {
						...
						couponPanel = new Ext.Panel( {
							renderTo : 'addShopGoodsSellingInfo-grid',
					...
					doAfter : function(data){//上传后的数据返回
					 var json = Ext.util.JSON.decode(data);
                         ...
                         parent.ChannelShopGoodsSellingInfoVo.refresh();
					},
					//删除整个panel里面被选择的行数据
			...
			function upload (){ //导入excel以后就与调整单绑定传入数据库保存
				...
				FormEditWin.showAddDirWin(null,basePath+"/fileUpload.html?uploadUrl="+basePath+"/custom/shopGoodsSellingInfo/inportGoodsSellingInfo.spmvc"+params,"picADWinID","上传",480,300);
			}
			...
			function addChannelGoodsTicket() { 
				      ...
				      var excuteTime = $("#channelGoodsTicket_excuteTime").val(); //执行时间
				      var time = new Date().format('Y-m-d H:i:s');
				      if(isTimming ==""){
							 alertMsg("结果", "请选择执行类型！");
							 return;
	  ...
	  function runChannelGoodsTicket() {//点击查看/更新页面展示				
					...
					if (isTiming && isTiming != '') {
						Ext.getCmp('channelGoodsTicket.isTiming').setValue($("#addNewIsTiming").val());
					}
