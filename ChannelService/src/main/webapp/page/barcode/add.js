		...
		.onReady(function() {
				...
				return {
					init : function() {
						...
						tbar = [
								...
								'<a style="color:red" href = "'+ basePath + '/custom/fileUpload/fileDownloadCsv.spmvc?filePath=barcode.csv'
										+ '">模板下载</a>' ];
					...
					doAfter : function(data) {// 上传后的数据返回
						...
						parent.ChannelShopBarCodeVo.refresh();
			...
			function upload() { // 导入excel以后就与调整单绑定传入数据库保存
				...
				FormEditWin.showAddDirWin(null, basePath
						...
						+ "custom/barcode/import.spmvc" + params, "picADWinID",
						"上传", 480, 300);
			}
			...
			function addChannelGoodsTicket() {
				...
				if (excuteTime == null || excuteTime == "") {
					...
					path = basePath
							+ '/custom/ticketInfo/addChannelGoodsAndTickets.spmvc?excutTimeStr='+excuteTime;
				}
				// 生成调整单
				Ext.Ajax.request({
					waitMsg : '请稍等.....',
