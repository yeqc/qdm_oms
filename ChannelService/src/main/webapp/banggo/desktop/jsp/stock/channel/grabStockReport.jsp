		...
		<script>
			...
			Ext.onReady(function() {
	            ...
	            store.on("load", function(obj,record) {
	            	...
	            	body.unmask();
	            	if (flag == 2) {
	            		var skuCount = localChangeIds.length;
	           			var count = record.length;
	           			
		            	for (var i = 0; i < count; i++) {
		            		var temp = record[i];
		            		var sid = temp.get('sid');
		            		
		            		var check = false;
		            		if (skuCount > 0) {
		            			for (var j = 0; j < skuCount; j++) {
		            				if (sid == localChangeIds[j]) {
		            					check = true;
		            		...
		            		if (!check) {
		            			if (batchChangeStock) {
		            				...
		            				temp.set('changeStock', temp.get("realStock"));
		            			}
		            		}
		            	}
	            	}
				...
				var columns = [
					...
					{header:'渠道',dataIndex:'channelName',align:"center",sortable:true,renderer:function(value, metadata, record){
						return getLeftFormate(value);
					}},
					...
					{header:'实际强占库存',dataIndex:'stock',align:"center",sortable:true,renderer:function(value, metadata, record){
						return getRightFormate(value);
					}},
					{header:'已使用的强占库存',dataIndex:'frozenStock',width:120,align:"center",sortable:true,renderer:function(value, metadata, record){
						return getRightFormate(value);
					}},
					{header:'强占库存调整',dataIndex:'changeStock',align:"center", editor: new fm.NumberField({
		                allowBlank: false,
		                allowNegative: false,
		                maxValue: 100000
				...
				var foot_tb = new Ext.Toolbar({
					items:[
						{xtype : "tbfill"},
						{xtype: 'tbtext', text: '强占库存调整:'},
						{id: 'batchKValue',name:'batchKValue',xtype:'numberfield',width:100,emptyText:'强占库存调整'},
						"-",
						{id:"batch", text:"确定", iconCls:"add", handler:function(){setCommonBatchValue(flag);}},
						"-",
				...
				function setCommonBatchValue(flag) {
					
					// delete local data
					localChangeIds = [];
					localChangeStock = [];
					
					if(!validateBatchKValue(flag))
						return;
					
					setBatchValue(flag);
				}
				
				// 判断修改的抢占库存值不能小于已使用的库存值,0除外
				function validateBatchKValue(flag) {
					...
					if ((value || value.length > 0) && value > 0) {
						if (flag == 2) {
							...
							if (count > 0) {
								var maxfrozenStock = 0;
								var maxfrozenSku = '';
								// 给记录设置值
								for (var i = 0; i < count; i++) {
									var record = store.getAt(i);
									var frozenStock = record.get('frozenStock');
				            		
									if(maxfrozenStock < frozenStock) {
										maxfrozenStock = frozenStock;
										maxfrozenSku = record.get('sku');
									}
								}
								
								if(value < maxfrozenStock) {
									createAlert("信息提示", "强占库存调整值(0除外)不能小于这批数据中最大的已使用强占库存值"
											+maxfrozenStock+"(11位码:"+maxfrozenSku+")");
									return false;
								}
							} else {
							...
							if (count > 0) {
								...
								var maxfrozenSku = '';
								for (var i = 0; i < count; i++) {
									var frozenStock = tempUserData[i].frozenStock;
				            		
									if(maxfrozenStock < frozenStock) {
										maxfrozenStock = frozenStock;
										maxfrozenSku = tempUserData[i].sku;
									}
								}
								if(value < maxfrozenStock) {
									createAlert("信息提示", "强占库存调整值(0除外)不能小于这批数据中最大的已使用强占库存值"
				...
				// 列表
				var model = getEditGridPanelModel("grid0", "强占库存调整列表", cm, store, limit, Ext.get("grid"),sm,foot_tb,tb);
				...
				function validateEdit(e) {
					...
					if(value < frozenStock && value != 0) {
						createAlert("信息提示", "强占库存调整值(0除外)不能小于这个sku的已使用强占库存值"
								+frozenStock);
						record.set('changeStock', stock);
						return false;
					}
				...
				function afterEdit(e) {
					...
					if (flag == 2) {
						if (!localChangeIds.contains(sid)) {
							localChangeIds.push(sid);
							localChangeStock.push(newValue);
						} else {
							for (var i = 0, leng = localChangeIds.length; i < leng; i++) {
								if (localChangeIds[i] == sid) {
									localChangeStock[i] = newValue;
									break;
								}
							}
						}
					...
					for(var i = 0 ; i < leng; i++){
						var record = row[i];
	    	  			...
	    	  			for (var j = 0; j < tempUserData.length; j++) {
							...
							if (temp.sku == sku && temp.channelCode == channelCode) {
							...
							Ext.Msg.confirm("警告",msg,function(btn){
	                        	if(btn == "yes"){
	                        		if (localChangeStock.length > 0 || batchChangeStock) {
	            						...
	            						params.stockChangeType = 1;
	            						params.ids = localChangeIds.join(",");
	                        	...
	                        	if(btn == "yes"){
	                        		...
	                        		for (var i = 0; i < count; i++) {
	    								...
	    								if (changeStock || changeStock == 0) {
		    								...
		    								channelCodes.push(record.channelCode);
	                        		...
	                        		if (skus.length > 0 && stocks.length > 0) {
	    								...
	    								params.channelCodes = channelCodes.join(",");
	    								Ext.getBody().mask('Loading', 'x-mask-loading');
	    								createAjax(path + "/scStockChange/doSaveScStockChange.htm", doSuccess, doFail, params);
	    							} else {
	    								if (stocks.length == 0) {
			...
			function setData(list, flag) {
				...
				for (var i = 0; i < count; i++) {
					...
					if (!isExsit) {
						if (!flag) {
							record.changeStock = record.stock;
						}
