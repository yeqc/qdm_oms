	...
	<head>
		...
		<script>
			...
			Ext.onReady(function() {
				...
				function downLoad() {
					...
					if (check) {
	            		...
	            		postForm.action = path + "/scstock/downLoadCommonList.htm";
		         		...
		         		name.setAttribute("value", "virtualStockList");
				...
				// 上传
				function upload() {
					var url = "../common/saleStockUpload.jsp?state=2&type=2";
		    	  		...
		    	  		for(var i = 0 ; i < leng; i++){
							...
							var bl = false;
		    	  			...
		    	  			if (!bl) {
		    	  				...
		    	  				data.changeStock = data.virStock;
				...
				function synch() {
					...
					if (length == 0) {
						...
						Ext.Msg.confirm("警告",msg,function(btn){
	                    	if(btn == "yes"){
								var url = "virtualStockReport.jsp?flag=1";
								showStockWindow(url);
	                    	}
						});
					}
				...
				function batchSynch() {
					...
					Ext.Msg.confirm("警告",msg,function(btn){
                    	if(btn == "yes"){
							var url = "virtualStockReport.jsp?flag=2";
							showStockWindow(url);
                    	}
					});
				}
			...
			function showFileWindow(data) {
				tempLocalData = data;
				var url = "virtualStockReport.jsp?flag=3";
				showStockWindow(url);
			}
			
			function showStockWindow(url) {
				storeWindow = showWindow('virtualStockReport','虚拟库存调整单', url, 800, 500, 50, 20);
				storeWindow.show();
			}
		</script>
	</head>
