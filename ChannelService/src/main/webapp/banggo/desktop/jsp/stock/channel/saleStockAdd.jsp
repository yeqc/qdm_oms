		...
		<script>
			...
			Ext.onReady(function() {
				...
				function getQuery() {
			    	...
			    	store.setBaseParam("saleStock", 1); // 不需要查询销售占用
			    	store.setBaseParam("os",1); // 查询商品信息
				...
				function downLoad() {
					...
					if (check) {
	            		...
	            		postForm.action = path + "/scstock/downLoadCommonList.htm";
		         		...
		         		name.setAttribute("value", "saleStockList");
				...
				// 上传
				function upload() {
					var url = "../common/saleStockUpload.jsp?state=2&type=3";
				...
				function synch() {
					...
					if (length == 0) {
						...
						Ext.Msg.confirm("警告",msg,function(btn){
	                    	if(btn == "yes"){
								var url = "saleStockReport.jsp?flag=1";
								showStockWindow(url);
	                    	}
						});
					}
				...
				function batchSynch() {
					...
					Ext.Msg.confirm("警告",msg,function(btn){
                    	if(btn == "yes"){
							var url = "saleStockReport.jsp?flag=2";
							showStockWindow(url);
                    	}
					});
				}
			...
			function showFileWindow(data) {
				tempLocalData = data;
				var url = "saleStockReport.jsp?flag=3";
				showStockWindow(url);
			}
			
			function showStockWindow(url) {
				storeWindow = showWindow('saleStockReport','可售库存调整单', url, 800, 500, 50, 20);
				storeWindow.show();
			}
		</script>
	</head>
