		...
		<script>
			...
			Ext.onReady(function() {
				...
				if (state == 0) {
					...
					var foot_tb = new Ext.Toolbar({
						items:[
							...
							{id: 'batchKValue',name:'batchKValue',xtype:'numberfield',width:100,emptyText:'虚拟库存调整'},
				   			"-",
							{id:"batch", text:"确定", iconCls:"add", handler:function(){setBatchValue(2);}},
							"-",
					...
					// 列表
					var model = getEditGridPanelModel("grid0", "虚拟库存调整列表", cm, store, 0, Ext.get("grid"),sm,foot_tb,tb);
					grid = createEditGridPanelByModel(model);
					grid.on('afteredit', afterEdit, this);
				} else if (state == 1) {
					// 列表
					var model = getGridPanelModel("grid0", "虚拟库存调整列表", columns, store, 0, Ext.get("grid"),sm);
					model.tbar = tb;
				...
				// 关闭 
				function close() {
					parent.closeTab("virtualStockList-" + stockChangeNo);
				}
