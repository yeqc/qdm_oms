...
function winOut() {
	...
	Ext.MessageBox.confirm( "请确认", "您确定要注销登录吗?", function(button,text){   
    	...
    	//	var form = document.form;
    		...
    		//alert(path);
    		//exitPath
    	//	form.action =exitPath;
			//form.action = path + logout;
		//	form.submit();
    		
    		window.location.href=exitPath+'/logout';
    	} 
	} );   
}
...
function menuOpen(obj) {
	if (obj.type == 1) {
		// 树形菜单
		var url = path + treePanel + "?type=" + obj.id + "&flag=" + obj.type+"&shopCode="+obj.shopCode;
...
function setBackGround() {
	...
	if (backGround) {
		...
		setBackGroundToCookie("backg.jpg");
	}
}

// 设置背景，并将地址保存到cookie中
function setBackGroundToCookie(name,obj) {
	var service_path = path + imageServicePath;
	// 设置到cookie
	...
	if (obj) {
		...
		/*var url = service_path + backGroundFile + name;
