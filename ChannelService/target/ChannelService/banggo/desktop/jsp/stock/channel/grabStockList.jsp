...
<html xmlns="http://www.w3.org/1999/xhtml">
	<head>
		...
		<script>
			...
			// 记录手工执行的sku
			var localChangeSku = [];
			// 记录手工执行的sku的channelCode
			var localChannelCode = [];
			...
			// 操作状态下拉框对象
			var stateComboBox = createComboBox(stateStore, 'scStockChangeSkuState', 'name', 'id', 'local', '');
			...
			Ext.onReady(function() {
					...
					items:[
						{xtype: 'compositefield',
							items: [
								...
								{xtype:'displayfield',value:'执行状态:', width:60},
								stateComboBox
							]
						}
					],
				...
				function downLoad() {
					...
					if (check) { 
	            		...
	            		postForm.action = path + "/scStockChange/downLoadCommonList.htm";
		         		...
		         		name.setAttribute("value", "scStockChannelSkuList");
				...
				store = new Ext.data.JsonStore({
					...
					fields:[
			      	...
			      	url:path + "/scStockChange/getChannelStockChangeSkuMap.htm",
					root:'list',
					totalProperty:'total'
				});
		            	...
		            	for (var i = 0; i < count; i++) {
		            		...
		            		if (skuCount > 0) {
		            			for (var j = 0; j < skuCount; j++) {
		            				if (sku == localChangeSku[j] && channelCode == localChannelCode[j]) {
		            					check = true;
				...
				var columns = [
					...
					{header:'执行状态',dataIndex:'status',align:"center",sortable:true,renderer:function(value, metadata, record){
						value = getState(value);
						return getLeftFormate(value);
					}},
					{header:'备注信息',dataIndex:'note',width:300,align:"center",sortable:true,renderer:function(value, metadata, record){
						return getLeftFormate(value);
					}},
				...
				if (state == 0) {
					...
					// 列表
					var model = getEditGridPanelModel("grid0", "强占库存调整列表", cm, store, 0, Ext.get("grid"),sm,foot_tb,tb);
					grid = createEditGridPanelByModel(model);
					grid.on('afteredit', afterEdit, this );
				} else if (state == 1) {
					// 列表
					var model = getGridPanelModel("grid0", "强占库存调整列表", columns, store, 0, Ext.get("grid"),sm);
					model.tbar = tb;
				...
				function afterEdit(e) {
					...
					var bl = false;
					for (var i = 0; i < localChangeSku.length; i++) {
						if (sku == localChangeSku[i] && channelCode == localChannelCode[i]) {
					...
					if (bl == false) {
						...
						localChannelCode.push(channelCode);
					}
				}
				getQuery();
				// 检索
				function getQuery() {
					...
					store.setBaseParam("scStockChangeSkuState", stockChangeState);
					store.load();
				}
						...
						Ext.Msg.confirm("警告",msg,function(btn){
                        	if(btn == "yes"){
                        		...
                        		// 提交批量更新
           						...
           						params.channelCodes = localChannelCode.join(",");
				...
				// 关闭 
				function close() {
					parent.closeTab("grabStockList-" + stockChangeNo);
				}
