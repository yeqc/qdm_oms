	...
	<head>
		...
		<script>
			...
			Ext.onReady(function() {
    			...
    			// 渠道下拉框对象
    			var channelComboBox = getChannelCombox("");
					...
					items:[
						{xtype: 'compositefield',
							items: [
								...
								{xtype:'displayfield',value:'渠道:', width:60},
								channelComboBox,
				...
				**/
				...
				var columns = [
					...
					{header:'商品名称',dataIndex:'name',align:"center",width:300,sortable:true,renderer:function(value, metadata, record){
						return getLeftFormate(value);
					}},	
				...
				function downLoad() {
					...
					if (check) {
	            		...
	            		postForm.action = path + "/scstock/downLoadCommonList.htm";
		         		...
		         		name.setAttribute("value", "grabStockList");
				...
				// 上传
				function upload() {
					var url = "../common/saleStockUpload.jsp?state=2&code=1&type=1";
					fileWindow = showWindow('saleStockUpload','文件上传',url,400,150);
					fileWindow.show();
				}
		    	  		...
		    	  		for(var i = 0 ; i < leng; i++){
							...
							var bl = false;
		    	  			...
		    	  			if (!bl) {
		    	  				...
		    	  				data.changeStock = data.realStock;
				...
				function synch() {
					...
					if (length == 0) {
						...
						Ext.Msg.confirm("警告",msg,function(btn){
	                    	if(btn == "yes"){
								var url = "grabStockReport.jsp?flag=1";
								showStockWindow(url);
	                    	}
						});
					}
				...
				function batchSynch() {
					...
					Ext.Msg.confirm("警告",msg,function(btn){
                    	if(btn == "yes"){
							var url = "grabStockReport.jsp?flag=2";
							showStockWindow(url);
                    	}
					});
				}
			...
			function showFileWindow(data) {
				tempLocalData = data;
				var url = "grabStockReport.jsp?flag=3";
				showStockWindow(url);
			}
			
			function showStockWindow(url) {
				storeWindow = showWindow('grabStockReport','强占库存调整单', url, 800, 500, 50, 20);
				storeWindow.show();
			}
		</script>
	</head>
