...
Ext.onReady(function() {
	...
	ChannelGoodsInfoPage = function() {
		...
		var tbar;
		var pageSize=15;
		var listUrl = basePath+"/custom/channelGoodsInfo/channelGoodsPage.spmvc?method=start";
		return {
			init : function() {
			channelGoodsInfoForm = new Ext.FormPanel(
				...
				channelGoodsInfoColumnGrid = new Ext.grid.ColumnModel( [
					...
					{id : 'channelPrice', align : "center", header : "价格", width : 50, dataIndex : 'channelPrice'},
					{id : 'barcodeChild', align : "center", sortable : true,renderer:chanageBarcodeChild, header : "颜色/尺码", width : 180, dataIndex : 'barcodeChild'},
					{id : 'isSync', align : "center", header : "库存同步状态", width : 100, dataIndex : 'isSync'},
					...
					{id : 'previewDetail', align : "center", sortable : true, header : "详情预览", width : 160, renderer: function() {
						return "<input type='button' value='详情预览' onclick='ChannelGoodsInfoPage.previewDetail();'/>";
					}}
				]);
				...
				channelGoodsInfoStore.on('beforeload', function(){
					var goodsName = Ext.getCmp('channelGoodsInfo.goodsName').getValue();
					var goodsSn = Ext.getCmp('channelGoodsInfo.goodsSn').getValue();
...
//							});
						...
						handler : function() {
							...
							FormEditWin.showAddDirWin("exprot_goods_win", basePath+ "/custom/shopBusinessGoods/exprotShopbusinessGoods.spmvc?shopCode="+getGlobalValue('shopCode')+"&channelCode="+getGlobalValue('channelCode')+"&status=4","picADWinID", "导出商品", 400, 300);
						}
					},'<a style="color:red" href = "'+ basePath + '/custom/fileUpload/fileDownloadCsv.spmvc?filePath=upload_goods.csv' + '">模板下载</a>'
...
//						}
			...
			search : function() {
				var searchParam = {};
				searchParam["goodsSn"] = Ext.getCmp("channelGoodsInfo.goodsSn").getValue();
				searchParam["goodsName"] = Ext.getCmp("channelGoodsInfo.goodsName").getValue();
