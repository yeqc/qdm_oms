Ext.onReady(function() {
		...
		return {
			init : function() {
				...
				var typeOptionCombo = new Ext.form.ComboBox({
					id : 'ChannelShopGoodsBarCodeVo.isTiming',
					...
					forceSelection : true,
					emptyText : '请选择执行类型',
					name : 'ChannelShopGoodsBarCodeVo.isTiming',
					editable : false,
					hiddenName : 'ChannelShopGoodsBarCodeVo.isTiming',
					triggerAction : 'all',
					fieldLabel : '执行类型',
					width : 150
				});
				...
				shopSelectOption = new Ext.form.ComboBox(
						{
							id : 'ChannelShopGoodsBarCodeVo.shopCode',
				...
				var payStatusOption = new Ext.form.ComboBox(
						{
							id : 'ChannelShopGoodsBarCodeVo.ticketStatus',
							store : new Ext.data.SimpleStore({
								data : [ [ '0', '未审核' ],
										[ '1', '已审核' ], [ '2', '已移除' ],
										[ '3', '已执行' ] ],
							...
							forceSelection : true,
							emptyText : '请选择单据状态',
							name : 'ChannelShopGoodsBarCodeVo.ticketStatus',
							editable : false,
							hiddenName : 'ChannelShopGoodsBarCodeVo.ticketStatus',
							triggerAction : 'all',
							fieldLabel : '单据状态',
							width : 150
						});
								...
								items : [
										{
											...
											items : [
													shopSelectOption,
													{
														id : 'ChannelShopGoodsBarCodeVo.ticketCode',
														xtype : 'textfield',
														fieldLabel : '调整单编号',
														name : 'ChannelShopGoodsBarCodeVo.ticketCode',
														blankText : '调整单编号不能为空',
														width : '142'
													} ]
										},
				...
				channelGoodsTicketColumnGrid = new Ext.grid.ColumnModel(
						[
								...
								},
								{
									...
									dataIndex : 'ticketStatus',
									renderer:function(e){
									       var msg = "<input type='button' value='导出调整单' onclick='ChannelShopBarCodeVo.exportRecord();' />"; 			
									        if(e=='3'){
									        	msg = "<input type='button' value='导出执行结果' onclick='ChannelShopBarCodeVo.exportRecord();' />";
									        }
									       return msg;
									}
				...
				tbar = [
						{
							...
							handler : function() {
								FormEditWin
										.showAddDirWin(
												"popWins",
												basePath
														+ "/custom/barcode/addShopGoodsPricePage.spmvc",
												"pops_message_winID",
												"生成调整单", 860, 560);
							}
						}, {
							...
							iconCls : 'add',
							handler : function() {
								ChannelShopBarCodeVo.reviewObject();
						 ...
						 * ChannelShopBarCodeVo.unReviewObject(); } }
						 **********************************************/
						, {
							...
							iconCls : 'delete',
							handler : function() {
								ChannelShopBarCodeVo.deleteObject();
							}
						} ];
								...
								'rowdblclick' : function(thisgrid,
									...
									FormEditWin
											.showAddDirWin(
													"popWins",
													basePath
															+ "/custom/barcode/updateOrSearchTicketInfo.spmvc?id="
															+ id,
													"pop_message_winID",
													"查看/更新调整单", 860,
													560);
			...
			search : function() {
				...
				outStock["isTiming"] = Ext.getCmp(
						"ChannelShopGoodsBarCodeVo.isTiming").getValue();
				outStock["shopCode"] = Ext.getCmp(
						"ChannelShopGoodsBarCodeVo.shopCode").getValue();
				outStock["ticketStatus"] = Ext.getCmp(
						"ChannelShopGoodsBarCodeVo.ticketStatus")
						.getValue();
				outStock["ticketCode"] = Ext.getCmp(
						"ChannelShopGoodsBarCodeVo.ticketCode")
						.getValue(); // 单据编号
				channelGoodsTicketGrid.store.baseParams = outStock;
				// Ext.Msg.alert(Ext.getCmp("ChannelShopGoodsBarCodeVo.ticketCode").getValue());
	...
	function runChannelGoodsTicketList() {
		ChannelShopBarCodeVo.init();
		ChannelShopBarCodeVo.show();
		setResize();
		shopSelectOption.store.on('load',
				function(store, record, opts) {
					Ext.getCmp('ChannelShopGoodsBarCodeVo.shopCode')
							.setValue(shopCodeStr); // 获取登录人的店铺code
				});
