Ext
		.onReady(function() {
			...
			ChannelGoodsFreightPayerVo = function() {
				...
				return {
					init : function() {
						...
						var typeOptionCombo = new Ext.form.ComboBox({
							...
							forceSelection : true,
							emptyText : '请选择执行类型',
							name : 'FreightPayerVo.isTiming',
							editable : false,
							hiddenName : 'FreightPayerVo.isTiming',
							triggerAction : 'all',
							fieldLabel : '执行类型',
							width : 150
						});
						...
						var payStatusOption = new Ext.form.ComboBox({
							id : 'FreightPayerVo.ticketStatus',
							store : new Ext.data.SimpleStore({
								data : [ [ '0', '未审核' ], [ '1', '已审核' ],
										[ '2', '已移除' ], [ '3', '已执行' ] ],
								fields : [ 'text', 'filed' ]
							...
							forceSelection : true,
							emptyText : '请选择单据状态',
							name : 'FreightPayerVo.ticketStatus',
							editable : false,
							hiddenName : 'FreightPayerVo.ticketStatus',
							triggerAction : 'all',
							fieldLabel : '单据状态',
							width : 150
						});
										...
										items : [
												{
													...
													items : [
															shopSelectOption,
															{
																id : 'FreightPayerVo.ticketCode',
																xtype : 'textfield',
																fieldLabel : '调整单编号',
																name : 'FreightPayerVo.ticketCode',
																blankText : '调整单编号不能为空',
																width : '142'
															} ]
												},
						...
						channelGoodsTicketColumnGrid = new Ext.grid.ColumnModel(
								[
										...
										},
										{
											...
											dataIndex : 'ticketStatus',
											renderer : function(e) {
												var msg = "<input type='button' value='导出调整单' onclick='ChannelGoodsFreightPayerVo.exportRecord();' />";
												if (e == '3') {
													msg = "<input type='button' value='导出执行结果' onclick='ChannelGoodsFreightPayerVo.exportRecord();' />";
												}
												return msg;
											}
										} ]);
						...
						channelGoodsTicketProxy = new Ext.data.HttpProxy(
								{
									url : basePath
											+ "/custom/shopGoods/shopGoodsUpDownList.spmvc",
									method : "post"
								});
						// Reader 读json中数据
						...
						tbar = [
									...
									handler : function() {
										FormEditWin
												.showAddDirWin(
														"popWins",
														basePath
																+ "/custom/freightpayer/addShopGoodsPricePage.spmvc",
														"pops_message_winID",
														"生成调整单", 860, 560);
									}
								},
								{
									...
									iconCls : 'add',
									handler : function() {
										ChannelGoodsFreightPayerVo
												.reviewObject();

									}
								}
								/***********************************************
								 * ,{ id : 'message_grid_tBar@unSure', text :
								 * '批量未审核', tooltip : 'delete', iconCls :
								 * 'delete', handler:function(){
								 * ChannelGoodsFreightPayerVo.unReviewObject(); } }
								 **********************************************/
								,
								{
						...
						channelGoodsTicketGrid = new Ext.grid.GridPanel(
								{
									...
									autoWidth : true,
									height : 500,
									title : '店铺商品运费承担方式批量调整',
									store : channelGoodsTicketStore,
									id : 'addpriceForm_gridss_id',
									...
									bbar : new IssPagingToolbar(
											channelGoodsTicketStore, pageSize),
									listeners : {
										'rowdblclick' : function(thisgrid,
											...
											FormEditWin
													.showAddDirWin(
															"popWins",
															basePath
																	+ "/custom/freightpayer/updateOrSearchTicketInfo.spmvc?id="
																	+ id,
															"pop_message_winID",
															"查看/更新调整单", 860,
															560);
					...
					checkOrUpdate : function() {// 查看编辑按钮，需要把调整单号作为参数传递过去
						...
						var selModel = channelGoodsTicketGrid
								.getSelectionModel();
					...
					exportRecord : function() {// 导出本行的数据
						...
						if (selModel.hasSelection()) {
							...
							if (id != "") {// 批量删除
								var path = basePath
										+ '/custom/shopGoods/exportChannelGoodsTickets.spmvc?id='
										+ id + "&type=freightpayer";
							...
							if (ids != "") {// 批量审核
								Ext.Ajax
										.request({
											waitMsg : '请稍等.....',
											url : basePath
													+ '/custom/shopGoods/reviewChannelGoodsTicket.spmvc',
											method : 'post',
											params : {
												ids : ids,
												ticketStatus : 0
					...
					reviewObject : function() {
						...
						if (selModel.hasSelection()) {
							...
							if (ids != "") {// 批量审核
								selectId = ids;
								// 选择邮费模板
								choiceTemplateForm.form.reset();
								win.setTitle("请选择邮费模板");
								win.show();
							}
						} else {
					...
					show : function() {
						...
						couponPanel = new Ext.Panel({
							renderTo : 'freightpayer-list-grid',
							layout : 'column',
							items : [ channelGoodsTicketForm,
									channelGoodsTicketGrid ]
						}).show();
					},
					refresh : function() {
					...
					search : function() {
						var outStock = {};
						outStock["isTiming"] = Ext.getCmp(
								"FreightPayerVo.isTiming").getValue();
						outStock["shopCode"] = Ext.getCmp(
								"FreightPayerVo.shopCode").getValue();
						outStock["ticketStatus"] = Ext.getCmp(
								"FreightPayerVo.ticketStatus").getValue();
						outStock["ticketCode"] = Ext.getCmp(
								"FreightPayerVo.ticketCode").getValue(); // 单据编号
						channelGoodsTicketGrid.store.baseParams = outStock;
						// Ext.Msg.alert(Ext.getCmp("FreightPayerVo.ticketCode").getValue());
						channelGoodsTicketGrid.store.load({
							params : {
								start : 0,
								limit : pageSize,
								ticketType : 6
			...
			// 执行类型选择框
			var templateListSelectOption = new Ext.form.ComboBox({
				id : 'template_id',
				store : new Ext.data.Store({
					proxy : new Ext.data.HttpProxy(
							{
								url : basePath
										+ '/custom/freightpayer/postTemplate.spmvc?shopCode=' + shopCodeStr,
								method : 'GET'
							}),
					reader : new Ext.data.JsonReader(
							{
			...
			var choiceTemplateForm = new Ext.FormPanel({
				...
				items : [ {
					...
					border : false,
					labelSeparator : '：',
					items : [ templateListSelectOption ]
				} ],
				buttonAlign : 'center',
				buttons : [ {
					text : '提交',
					handler : function() {
						if (!choiceTemplateForm.getForm().isValid()) {
			...
			var win = new Ext.Window({
				...
				animCollapse : true,
				items : [ choiceTemplateForm ]
			});

			function runChannelGoodsTicketList() {
				ChannelGoodsFreightPayerVo.init();
				ChannelGoodsFreightPayerVo.show();
				setResize();
				shopSelectOption.store.on('load',
						function(store, record, opts) {
							Ext.getCmp('FreightPayerVo.shopCode').setValue(
									shopCodeStr); // 获取登录人的店铺code
						});
			...
			function setResize() {
				...
				channelGoodsTicketGrid
						.setHeight(clientHeight - formHeight - 50);
