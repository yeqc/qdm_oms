		...
		.onReady(function() {
				...
				return {
					init : function() {
						...
						tbar = [
								...
								'<a style="color:red" href = "'+ basePath + '/custom/fileUpload/fileDownloadCsv.spmvc?filePath=freightpayer.csv'
										+ '">模板下载</a>' ];
					...
					show : function() {
						...
						couponPanel = new Ext.Panel({
							renderTo : 'add-freightpayer-grid',
					...
					doAfter : function(data) {// 上传后的数据返回
						...
						parent.ChannelGoodsFreightPayerVo.refresh();
			...
			function upload() { // 导入excel以后就与调整单绑定传入数据库保存
				...
				FormEditWin.showAddDirWin(null, basePath
						...
						+ "custom/freightpayer/import.spmvc" + params,
