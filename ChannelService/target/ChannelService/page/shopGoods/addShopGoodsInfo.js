	...
	TicketInfo= function() {
				...
				return {
					...
					var isTimingSelectOption = new Ext.form.ComboBox( {
							...
							listeners:{
									'select': function(){
											if(Ext.getCmp("channelGoodsTicket.isTiming").getValue() == '0')
											{
						...
						ticketInfoColumnGrid = new Ext.grid.ColumnModel( [
								...
								{id : 'option', align : "center", sortable : true, header : "操作", width : 120, dataIndex : 'safePrice',renderer:function(value, metadata, record, rowIndex) {
									var val = record.get('ticketStatus');
									var html = "<input type='button' value='预览详情' onclick='TicketInfo.preview();' />";
									return html;
								}}
						]);
						...
						ticketInfoStore.on('beforeload', function(){
							var keyWord = Ext.getCmp("channelGoodsTicket.ticketCode").getValue();
						...
						tbar = [{
							...
							},'<a style="color:red" href = "'+ basePath + '/custom/fileUpload/fileDownloadCsv.spmvc?filePath=channel_goods.csv' + '">模板下载</a>'
							];
					...
					doAfter : function(data){//上传后的数据返回
						...
						var json = Ext.util.JSON.decode(data);
							 ...
							 parent.ChannelGoodsInfoVo.refresh();
					},
					//删除整个panel里面被选择的行数据
					deleteTicketInfo :function(){
						...
						preview : function(){ //点击预览按钮
							var selModel = ticketInfoGrid.getSelectionModel();
							if (selModel.hasSelection()) {
								var records = selModel.getSelections();
								var goodsSn = records[0].get("goodsSn");
								if(goodsSn != ""){
									var url = basePath+"custom/channelGoodsInfo/previewDetail.spmvc?goodsSn="+goodsSn+"&shopCode="+shopCodeStr
									+"&channelCode="+chanelCodeStr + "&ticketType=2";
									window.open(url);
								}else{
			...
			function upload (){ //导入excel以后就与调整单绑定传入数据库保存
				...
				var shopCode = Ext.getCmp("channelGoodsTicket.shopCode").getValue(); //店铺名称code
				var params = "&params=ticketCode:"+ticketCode+";shopCode:"+shopCode+";excetTimes:"+
				...
				FormEditWin.showAddDirWin(null,basePath+"/fileUpload.html?uploadUrl="+basePath+"custom/ticketInfo/uploadTicketFile.spmvc"+params,"picADWinID","上传",480,300);
			}
			...
			function addChannelGoodsTicket() { 
					  var ticketCode  = Ext.getCmp("channelGoodsTicket.ticketCode").getValue(); //单据编号
					  var shopCode = Ext.getCmp("channelGoodsTicket.shopCode").getValue(); //店铺名称code
					  var isTimming = Ext.getCmp("channelGoodsTicket.isTiming").getValue(); //是否定时执行
						 ...
						 Ext.Ajax
								.request( {
									...
									params : {
										...
										channelCode:Ext.getCmp("OutChannelShop.channelCode").getValue(),
										shopCode:shopCode,
										ticketType: 2, //更新商品信息
		...
		function runChannelGoodsTicket() {//点击查看/更新页面展示		
			...
			Ext.getCmp('channelGoodsTicket.ticketCode').setValue($("#addNewTicketCode").val());
			shopSelectOption.store.on('load', function(store, record, opts) {
				Ext.getCmp('channelGoodsTicket.shopCode').setValue(shopCodeStr); 
			});
					shopSelectOption.store.load();
					channelInfoCombo.store.on('load', function(store, record, opts) {
						Ext.getCmp('OutChannelShop.channelCode').setValue(chanelCodeStr); 
					});
				...
				//	Ext.getCmp('channelGoodsTicket.shopCode').setValue($("#addNewShopCode").val());
					...
					Ext.getCmp('channelGoodsTicket.isTiming').setValue("0");
					...
					Ext.getCmp('channelGoodsTicket.isTiming').setValue(isTiming);
					if(isTiming == '0'){
						 Ext.getCmp("excuteTimeNo").disable(); 
						 $("#channelGoodsTicket_excuteTime").val("") ;//执行时间
					}
