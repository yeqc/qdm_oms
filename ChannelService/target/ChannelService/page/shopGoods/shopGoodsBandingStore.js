	...
	//  var shopNameStr = GetCookie(name);
				...
				return {
					init : function() {
					 ...
					 var typeOptionCombo = new Ext.form.ComboBox( {
							id : 'channelGoodsTicketVo3.isTiming',
							...
							forceSelection : true,
							emptyText : '请选择执行类型',
							name : 'channelGoodsTicketVo3.isTiming',
							editable : false,
							hiddenName : 'channelGoodsTicketVo3.isTiming',
							triggerAction : 'all', 
							fieldLabel : '执行类型',
							width : 150
						});
					 ...
					 //经营店铺选择框
					  shopSelectOption = new Ext.form.ComboBox( {
							id : 'channelGoodsTicketVo3.shopCode',
					 ...
					 var payStatusOption = new Ext.form.ComboBox( {
							id : 'channelGoodsTicketVo3.ticketStatus',
							store : new Ext.data.SimpleStore( {
							...
							forceSelection : true,
							emptyText : '请选择单据状态',
							name : 'channelGoodsTicketVo3.ticketStatus',
							editable : false,
							hiddenName : 'channelGoodsTicketVo3.ticketStatus',
							triggerAction : 'all', 
							fieldLabel : '单据状态',
							width : 150
						});
									...
									items : [ {
										...
										items : [{
													...
													items : [shopSelectOption,{
														id : 'channelGoodsTicketVo3.ticketCode',
														xtype : 'textfield',
														labelAlign : 'left',
														fieldLabel : '调整单编号',
														name : 'channelGoodsTicketVo3.ticketCode', 
														blankText : '调整单编号不能为空',
														width : '142'
													}]
												},{
				        ...
				        channelGoodsTicketStore.on('beforeload', function(){
				            Ext.apply(this.baseParams, {ticketType: 10, shopCode:shopCodeStr});   
				        });
						...
						tbar = [{
							...
							iconCls : 'add',
							handler : function() {
							FormEditWin.showAddDirWin("popWins",basePath+"/custom/shopGoods/addShopGoodsBandingStore.spmvc","pop_message_winID","生成调整单",860,560);
							}
						    },{ 
							...
							listeners : {
					            'rowdblclick': function(thisgrid ,rowIndex ,e){
									...
									var id = record.data['id']; 
									 FormEditWin.showAddDirWin("popWins",basePath+"/custom/ticketInfo/updateOrSearchGoodsStoreBandingTicketInfo.spmvc?id="+id,"pop_message_winID","查看/更新调整单",860,560);
					            }
					        }
						});
					...
					exportRecord:function(){//导出本行的数据
						...
						if (selModel.hasSelection()) {
							...
							var id = records[0].get("id");														
							if(id != ""){
								 var path = basePath + '/custom/shopGoods/exportChannelGoodsTickets.spmvc?id='+id+"&type=syncstock";
				   ...
				   deleteObject : function() { //删除整个panel里面被选择的行数据
					...
					search : function() {
					  var outStock = {};					      
					      outStock["isTiming"] = Ext.getCmp("channelGoodsTicketVo3.isTiming").getValue();
					      outStock["shopCode"] = Ext.getCmp("channelGoodsTicketVo3.shopCode").getValue();
						  outStock["ticketStatus"] = Ext.getCmp("channelGoodsTicketVo3.ticketStatus").getValue();
					      outStock["ticketCode"] = Ext.getCmp("channelGoodsTicketVo3.ticketCode").getValue(); //单据编号
						  channelGoodsTicketGrid.store.baseParams = outStock;
					//	  Ext.Msg.alert(Ext.getCmp("channelGoodsTicketVo3.ticketCode").getValue());//1上下架单据类型
						  channelGoodsTicketGrid.store.load({params : {start : 0, limit : pageSize,ticketType: 10 }});
					} 
			...
			function clickResetButton() {
				...
				shopSelectOption.store.on('load', function(store, record, opts) {
 					Ext.getCmp('channelGoodsTicketVo3.shopCode').setValue(shopCodeStr); //获取登录人的店铺code
 				});
