		...
		.onReady(function() {
				...
				return {
					init : function() {
						...
						tbar = [
								...
								'<a style="color:red" href = "'+ basePath + '/custom/fileUpload/fileDownloadCsv.spmvc?filePath=decreaseAfterAuction.csv'
										+ '">模板下载</a>' ];
					...
					show : function() {
						...
						couponPanel = new Ext.Panel({
							renderTo : 'add-decreaseAfterAuction-grid',
					...
					doAfter : function(data) {// 上传后的数据返回
						...
						parent.ChannelShopDecreaseAfterAuctionVo.refresh();
			...
			function upload() { // 导入excel以后就与调整单绑定传入数据库保存
				...
				FormEditWin.showAddDirWin(null, basePath
						...
						+ "custom/decreaseAfterAuction/import.spmvc" + params,
			...
			function runChannelGoodsTicket() {// 点击查看/更新页面展示
				...
				var addNewIsTiming = $("#addNewIsTiming").val();
				if (addNewIsTiming != undefined && addNewIsTiming != null && addNewIsTiming != '') {
					Ext.getCmp('channelGoodsTicket.isTiming').setValue($("#addNewIsTiming").val());
				} else {
					Ext.getCmp('channelGoodsTicket.isTiming').setValue('0');
				}
				var addNewExcetTime = $("#addNewExcetTime").val();
				if (addNewExcetTime != undefined && addNewExcetTime != null && addNewIsTiming == "1") {
					$("#channelGoodsTicket_excuteTime").val(addNewExcetTime);// 执行时间
				} else {
					Ext.getCmp("excuteTimeNo").disable();
