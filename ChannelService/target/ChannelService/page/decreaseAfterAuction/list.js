Ext
		.onReady(function() {
			...
			ChannelShopDecreaseAfterAuctionVo = function() { 
				...
				return {
					init : function() {
						...
						var typeOptionCombo = new Ext.form.ComboBox({
							id : 'DecreaseAfterAuctionVo.isTiming',
							...
							forceSelection : true,
							emptyText : '请选择执行类型',
							name : 'DecreaseAfterAuctionVo.isTiming',
							editable : false,
							hiddenName : 'DecreaseAfterAuctionVo.isTiming',
							triggerAction : 'all',
							fieldLabel : '执行类型',
							width : 150
						});
						...
						shopSelectOption = new Ext.form.ComboBox(
								{
									id : 'DecreaseAfterAuctionVo.shopCode',
						...
						var payStatusOption = new Ext.form.ComboBox({
							id : 'DecreaseAfterAuctionVo.ticketStatus',
							store : new Ext.data.SimpleStore({
								data : [ [ '0', '未审核' ], [ '1', '已审核' ],
										[ '2', '已移除' ], [ '3', '已执行' ] ],
								fields : [ 'text', 'filed' ]
							...
							forceSelection : true,
							emptyText : '请选择单据状态',
							name : 'DecreaseAfterAuctionVo.ticketStatus',
							editable : false,
							hiddenName : 'DecreaseAfterAuctionVo.ticketStatus',
							triggerAction : 'all',
							fieldLabel : '单据状态',
							width : 150
						});
										...
										items : [
												{
													...
													items : [
															shopSelectOption,
															{
																id : 'DecreaseAfterAuctionVo.ticketCode',
																xtype : 'textfield',
																fieldLabel : '调整单编号',
																name : 'DecreaseAfterAuctionVo.ticketCode',
																blankText : '调整单编号不能为空',
																width : '142'
															} ]
												},
						...
						channelGoodsTicketColumnGrid = new Ext.grid.ColumnModel(
								[
										...
										},
										{
											...
											dataIndex : 'ticketStatus',
											renderer : function(e) {
												var msg = "<input type='button' value='导出调整单' onclick='ChannelShopDecreaseAfterAuctionVo.exportRecord();' />";
												if (e == '3') {
													msg = "<input type='button' value='导出执行结果' onclick='ChannelShopDecreaseAfterAuctionVo.exportRecord();' />";
												}
												return msg;
											}
										} ]);
						...
						tbar = [
								{
									...
									handler : function() {
										FormEditWin
												.showAddDirWin(
														"popWins",
														basePath
																+ "/custom/decreaseAfterAuction/addShopGoodsPricePage.spmvc",
														"pops_message_winID",
														"生成调整单", 860, 560);
									}
								}, {
									...
									iconCls : 'add',
									handler : function() {
										ChannelShopDecreaseAfterAuctionVo.reviewObject();
								 ...
								 * ChannelShopDecreaseAfterAuctionVo.unReviewObject(); } }
								 **********************************************/
								, {
									...
									iconCls : 'delete',
									handler : function() {
										ChannelShopDecreaseAfterAuctionVo.deleteObject();
									}
								} ];
										...
										'rowdblclick' : function(thisgrid,
											...
											FormEditWin
													.showAddDirWin(
															"popWins",
															basePath
																	+ "/custom/decreaseAfterAuction/updateOrSearchTicketInfo.spmvc?id="
																	+ id,
															"pop_message_winID",
															"查看/更新调整单", 860,
															560);
					...
					reviewObject : function() {
						...
						if (selModel.hasSelection()) {
							...
							if (ids != "") {// 批量审核
								Ext.getCmp("message_grid_tBar@update")
										.disable();
								Ext.Ajax
										.request({
											waitMsg : '请稍等.....',
					...
					show : function() {
						...
						couponPanel = new Ext.Panel({
							renderTo : 'decreaseAfterAuction-list-grid',
							layout : 'column',
					...
					search : function() {
						...
						outStock["isTiming"] = Ext.getCmp(
								"DecreaseAfterAuctionVo.isTiming").getValue();
						outStock["shopCode"] = Ext.getCmp(
								"DecreaseAfterAuctionVo.shopCode").getValue();
						outStock["ticketStatus"] = Ext.getCmp(
								"DecreaseAfterAuctionVo.ticketStatus").getValue();
						outStock["ticketCode"] = Ext.getCmp(
								"DecreaseAfterAuctionVo.ticketCode").getValue(); // 单据编号
						channelGoodsTicketGrid.store.baseParams = outStock;
						// Ext.Msg.alert(Ext.getCmp("DecreaseAfterAuctionVo.ticketCode").getValue());
			...
			function runChannelGoodsTicketList() {
				ChannelShopDecreaseAfterAuctionVo.init();
				ChannelShopDecreaseAfterAuctionVo.show();
				setResize();
				shopSelectOption.store.on('load',
						function(store, record, opts) {
							Ext.getCmp('DecreaseAfterAuctionVo.shopCode').setValue(
