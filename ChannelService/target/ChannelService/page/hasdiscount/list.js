Ext
		.onReady(function() {
			...
			ChannelShopHasDiscountVo = function() { 
				...
				return {
					init : function() {
						...
						var typeOptionCombo = new Ext.form.ComboBox({
							...
							forceSelection : true,
							emptyText : '请选择执行类型',
							name : 'HasDiscountVo.isTiming',
							editable : false,
							hiddenName : 'HasDiscountVo.isTiming',
							triggerAction : 'all',
							fieldLabel : '执行类型',
							width : 150
						});
						...
						var payStatusOption = new Ext.form.ComboBox({
							...
							forceSelection : true,
							emptyText : '请选择单据状态',
							name : 'HasDiscountVo.ticketStatus',
							editable : false,
							hiddenName : 'HasDiscountVo.ticketStatus',
							triggerAction : 'all',
							fieldLabel : '单据状态',
							width : 150
						});
										...
										items : [
												{
													...
													items : [
															shopSelectOption,
															{
																id : 'HasDiscountVo.ticketCode',
																xtype : 'textfield',
																fieldLabel : '调整单编号',
																name : 'HasDiscountVo.ticketCode',
																blankText : '调整单编号不能为空',
																width : '142'
															} ]
												},
						...
						channelGoodsTicketColumnGrid = new Ext.grid.ColumnModel(
								[
										...
										},
										{
											...
											dataIndex : 'ticketStatus',
											renderer : function(e) {
												var msg = "<input type='button' value='导出调整单' onclick='ChannelShopHasDiscountVo.exportRecord();' />";
												if (e == '3') {
													msg = "<input type='button' value='导出执行结果' onclick='ChannelShopHasDiscountVo.exportRecord();' />";
												}
												return msg;
											}
										} ]);
						...
						tbar = [
								{
									...
									handler : function() {
										FormEditWin
												.showAddDirWin(
														"popWins",
														basePath
																+ "/custom/hasdiscount/addShopGoodsPricePage.spmvc",
														"pops_message_winID",
														"生成调整单", 860, 560);
									}
								}, {
									...
									iconCls : 'add',
									handler : function() {
										ChannelShopHasDiscountVo.reviewObject();
								 ...
								 * ChannelShopHasDiscountVo.unReviewObject(); } }
								 **********************************************/
								, {
									...
									iconCls : 'delete',
									handler : function() {
										ChannelShopHasDiscountVo.deleteObject();
									}
								} ];
										...
										'rowdblclick' : function(thisgrid,
											...
											FormEditWin
													.showAddDirWin(
															"popWins",
															basePath
																	+ "/custom/hasdiscount/updateOrSearchTicketInfo.spmvc?id="
																	+ id,
															"pop_message_winID",
															"查看/更新调整单", 860,
															560);
					...
					show : function() {
						...
						couponPanel = new Ext.Panel({
							renderTo : 'hasdiscount-list-grid',
							layout : 'column',
					...
					search : function() {
						...
						outStock["isTiming"] = Ext.getCmp(
								"HasDiscountVo.isTiming").getValue();
						outStock["shopCode"] = Ext.getCmp(
								"HasDiscountVo.shopCode").getValue();
						outStock["ticketStatus"] = Ext.getCmp(
								"HasDiscountVo.ticketStatus").getValue();
						outStock["ticketCode"] = Ext.getCmp(
								"HasDiscountVo.ticketCode").getValue(); // 单据编号
						channelGoodsTicketGrid.store.baseParams = outStock;
						// Ext.Msg.alert(Ext.getCmp("HasDiscountVo.ticketCode").getValue());
			...
			function runChannelGoodsTicketList() {
				ChannelShopHasDiscountVo.init();
				ChannelShopHasDiscountVo.show();
				setResize();
				shopSelectOption.store.on('load',
						function(store, record, opts) {
							Ext.getCmp('HasDiscountVo.shopCode').setValue(
